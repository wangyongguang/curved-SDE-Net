...
import numpy as np
__all__ = ['SDENet_mnistBase','SDENet_mnistCurve']
...
class SDENet_mnistBase(nn.Module):
    def __init__(self,layer_depth,num_classes=10,dim=64):
        super(SDENet_mnistBase,self).__init__()
...
class SDENet_mnistCurve(nn.Module):
    def __init__(self, layer_depth, num_classes=10, dim=64,fix_points=[True,False,True]):
        ...
        self.downsampling_layers = nn.Sequential(
            curves.Conv2d(1, dim, 3, 1,fix_points=fix_points),
        ...
        # Initialize weights
        for m in [self.downsampling_layers.modules(),
                  self.drift1.modules(),self.drift2.modules(),
                  self.diffusion1.modules(),self.diffusion2.modules()]:
        ...
        for module in self.modules():
            if issubclass(module.__class__, curves.CurveModule):
